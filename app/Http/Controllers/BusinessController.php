...
if($request->action == 'Process')
{
			...
			$slug=preg_replace('/[^A-Za-z0-9-]+/', '-', $request->bussiness_name);
...
//                }
	    ...
	    if($request->action == 'Process')
	    {
			...
			if( is_array($request->file('upload_doc')) && count($request->file('upload_doc'))>0)
			{
				...
				if($dtls!='')
				{
					...
					$path                     = public_path('upload/attachment/' . $filename);
					$image->move(public_path('upload/attachment/'), $filename);
					$business_industries = new BusinessDocuments;
					$business_industries->business_id = $buss_id;
					$business_industries->document_name = $filename;
					$business_industries->save();
				}
			    }
			}
    ...
    public function login(Request $request){
        ...
        if($type == "Business" && $request->action == "Process"){
            ...
            if ($validator->fails()){
                ...
                $userDetails = Business::where("email","=",$email)->first();
    ...
    public function changepassword(){ 
        ...
        return view('business.changepassword',$data);
    }
    ...
    public function do_changepassword(Request $request){ 
        ...
        if($type == "Business" && $request->action == "Process"){ 
            ...
            if ($validator->fails()){  
                ...
                $id                         = $business_id; 
                $Business                   = Business::find($id);
		if (Hash::check($request->old_password, $Business->password))
                {
		    $Business->password         = $request->password;
		    $Business->save();
